...
- 接口功能：激活函数。返回与输入tensor shape相同的tensor，对输入tensor进行elementwise的计算。
- 计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用”aclnnTanhGetWorkspaceSize”或者”aclnnInplaceTanhGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用”aclnnTanh”或者”aclnnInplaceTanh”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      ...
      <td>输入</td>
      <td>公式中的输入self。</td>
      <td>shape需要与out一致，和out的数据类型满足<a href="../common/互推导关系.md" target="_blank">互推导关系</a>。</td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>公式中的out。</td>
      <td>shape需要与self一致，和self的数据类型满足<a href="../common/互推导关系.md" target="_blank">互推导关系</a>。</td>
...
- **返回值：**
  ...
  <tbody>
    ...
    <tr>
      <td>self和out的数据类型不满足<a href="../common/互推导关系.md" target="_blank">互推导关系</a>。</td>
    </tr>
    <tr>
