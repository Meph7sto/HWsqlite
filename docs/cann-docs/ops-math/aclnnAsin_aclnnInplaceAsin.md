...
```cpp
aclnnStatus aclnnInplaceAsinGetWorkspaceSize(
  aclTensor*          selfRef, 
  uint64_t*           workspaceSize, 
  aclOpExecutor**     executor) 
```
...
- **参数说明：**
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tbody>
      <tr>
        ...
        <td>输入</td>
        <td>当类型为INT8、INT16、INT32、INT64、UINT8、BOOL时，转化为FLOAT32进行运算，输出FLOAT32类型。</td>
        <td>支持空Tensor。</td>
  ...
  - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：self和out数据类型额外支持BFLOAT16，当类型为BFLOAT16时，转化为FLOAT16进行计算，再转化为BFLOAT16输出。
...
- **参数说明：**
  ...
  - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：selfRef数据类型支持BFLOAT16。
