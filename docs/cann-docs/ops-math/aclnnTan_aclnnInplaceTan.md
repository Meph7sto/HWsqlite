...
- 接口功能：它计算输入张量`self`中每个元素的正切值，并将结果存储在输出张量`out`中。
...
```Cpp
aclnnStatus aclnnInplaceTanGetWorkspaceSize(
...
- **返回值：**
  ...
  <tbody>
    ...
    <tr>
      <td>self不能转为out的数据类型，或self和out的数据类型不满足<a href="../common/互推导关系.md" target="_blank">互推导关系</a>。</td>
    </tr>
    <tr>
