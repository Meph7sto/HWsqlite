...
- **参数说明**：
  ...
  <tbody>
    <tr>
      ...
      <td class="tg-0pky">shape可以与clipValueMin进行<a href="../common/broadcast关系.md" target="_blank">broadcast</a>，数据类型与clipValueMin的数据类型需满足数据类型推导规则（参见<a href="../common/互转换关系.md" target="_blank">互转换关系</a>）。</td>
    ...
    <tr>
      ...
      <td class="tg-0lax">shape是self和clipValueMin<a href="../common/broadcast关系.md" target="_blank">broadcast</a>的结果，数据类型需要是self、clipValueMin推导之后可转换的数据类型。</td>
...
- **参数说明：**
  ...
  - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、Atlas 350 加速卡：
    - selfRef和clipValueMin数据类型需满足数据类型推导规则（参见[TensorScalar互推导关系](../common/TensorScalar互推导关系.md)）。
    - selfRef的数据类型不支持BOOL。
