...
- 接口功能：计算输入张量self中每个元素和输入标量other中对应位置元素的按位异或，输入self和other必须是整数或布尔类型，对于布尔类型，计算逻辑异或。
...
- **参数说明：**
  ...
  - Atlas 350 加速卡：self和other数据类型需满足[TensorScalar互推导关系](../common/TensorScalar互推导关系.md)。
...
- **参数说明：**
  ...
  - Atlas 350 加速卡：selfRef和other数据类型需满足[TensorScalar互推导关系](../common/TensorScalar互推导关系.md)，且需要是推导之后可转换的数据类型（参见[互转换关系](../common/互转换关系.md)）。
