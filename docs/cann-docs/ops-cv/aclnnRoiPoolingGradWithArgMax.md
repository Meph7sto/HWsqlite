...
算子执行接口为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnRoiPoolingGradWithArgMaxGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnRoiPoolingGradWithArgMax”接口执行计算。
...
- **返回值**
  ...
  返回aclnnStatus状态码，具体参见[aclnn返回码](../common/aclnn返回码_cv.md)。
