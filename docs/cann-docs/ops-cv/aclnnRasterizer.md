...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnRasterizerGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnRasterizer”接口执行计算。
...
- **返回值**
  ...
  aclnnStatus：返回状态码，具体参见[aclnn返回码](../common/aclnn%E8%BF%94%E5%9B%9E%E7%A0%81.md)。
