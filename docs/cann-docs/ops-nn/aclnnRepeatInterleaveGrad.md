...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnRepeatInterleaveGradGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnRepeatInterleaveGrad”接口执行计算。
...
- **参数说明：**
  ...
  - Atlas 350 加速卡 :yGrad的数据类型支持FLOAT16、BFLOAT16、FLOAT。
