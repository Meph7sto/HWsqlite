...
- 接口功能：完成二级量化mxfp4的矩阵乘计算，其中参数x2需为NZ格式，可通过[aclnnTransMatmulWeight](../ops-math/aclnnTransMatmulWeight.md)或[aclnnNpuFormatCast](../ops-math/aclnnNpuFormatCast.md)对format为ND的x2处理得到NZ格式。
- 计算公式
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnDualLevelQuantMatmulWeightNzGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDualLevelQuantMatmulWeightNz”接口执行计算。
