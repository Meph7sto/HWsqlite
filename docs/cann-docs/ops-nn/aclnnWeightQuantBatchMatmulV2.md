  ...
  <table style="table-layout: fixed; width: 1550px">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>输入</td>
        <td>表示在伪量化pergroup和mx<a href="../common/量化介绍.md" target="_blank">量化模式</a>下，对输入<code>weight</code>进行反量化计算的groupSize输入，描述一组反量化参数对应的待反量化数据量在Reduce方向的大小。</td>
        <td>当伪量化算法不为pergroup和mx<a href="../common/量化介绍.md" target="_blank">量化模式</a>时传入0。<br>当伪量化算法为pergroup<a href="../common/量化介绍.md" target="_blank">量化模式</a>时传入值的范围为[32, k-1]且值要求是32的倍数。<br>在mx<a href="../common/量化介绍.md" target="_blank">量化模式</a>，仅支持32。</td>
        <td>-</td>
        <td>-</td>
        <td>-</td>
        <td>-</td>
...
<summary><term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term></summary>
  ...
  - `weight`（aclTensor *, 计算输入）：维度支持2维，Reduce维度k需要与`x`的Reduce维度k大小相等。数据类型支持INT8、INT4、INT32，当`weight`[数据格式](../common/数据格式.md)为FRACTAL_NZ且数据类型为INT4或INT32时，或者当`weight`[数据格式](../common/数据格式.md)为ND且数据类型为INT32时，仅在INT4Pack场景支持，需配合`aclnnConvertWeightToINT4Pack`接口完成从INT32到INT4Pack的转换，以及从ND到FRACTAL_NZ的转换，[详情可参考样例](../ops-nn/aclnnConvertWeightToINT4Pack.md)，若数据类型为INT4，则`weight`的内轴应为偶数。[非连续的Tensor](../common/非连续的Tensor.md)仅支持转置场景。shape支持(k, n)，其中k表示矩阵第1维的大小，n表示矩阵第2维的大小。
  ...
  - `antiquantOffsetOptional`（aclTensor *, 计算输入）：数据类型支持FLOAT16、BFLOAT16、INT32，数据类型为FLOAT16、BFLOAT16时，数据类型要求和输入`x`的数据类型保持一致；数据类型为INT32类型时，数据范围限制为[-128, 127]，x仅支持FLOAT16，weight仅支持INT8，`antiquantScale`仅支持UINT64/INT64。[非连续的Tensor](../common/非连续的Tensor.md)仅支持转置场景。
  - `quantScaleOptional`（aclTensor *, 计算输入）：数据类型支持UINT64，[数据格式](../common/数据格式.md)支持ND。不支持[非连续的Tensor](../common/非连续的Tensor.md)。可选输入，当不需要时为空指针；对于不同的伪量化算法模式，支持的shape如下:
    - pertensor[量化模式](../common/量化介绍.md)：输入shape为(1,)或(1, 1)。
    - perchannel[量化模式](../common/量化介绍.md)：输入shape为(1, n)或(n,)。
  - `quantOffsetOptional`（aclTensor *, 计算输入）：数据类型支持FLOAT，[数据格式](../common/数据格式.md)支持ND。可选输入, 当不需要时为空指针；存在时shape要求与`quantScaleOptional`一致。不支持[非连续的Tensor](../common/非连续的Tensor.md)。
...
<summary><term>Atlas 推理系列产品</term></summary>
  ...
  - `x`（aclTensor *, 计算输入）：数据类型支持FLOAT16。shape支持2~6维，输入shape需要为(batch, m, k)，其中batch表示矩阵的批次大小，支持0~4维，m表示单个batch矩阵第1维的大小，k表示单个batch矩阵的第2维的大小，batch维度需要与`weight`的batch维度满足[broadcast关系](../common/broadcast关系.md)。当伪量化算法模式为pertensor[量化模式](../common/量化介绍.md)时，`m*k`不能超过512000000。
  - `weight`（aclTensor *, 计算输入）：维度支持2~6维，batch维度需要与`x`的batch维度满足[broadcast关系](../common/broadcast关系.md)，数据类型支持INT8。具体如下：
...
<summary>Atlas 350 加速卡</summary>
  ...
  - **公共约束**
    ...
    - 支持的量化模式：pertensor[量化模式](../common/量化介绍.md)、perchannel[量化模式](../common/量化介绍.md)、pergroup[量化模式](../common/量化介绍.md)和mx[量化模式](../common/量化介绍.md)。
    - `x`不支持转置，因此不支持[非连续Tensor](../common/非连续的Tensor.md)，weight仅转置场景支持非连续的Tensor；antiquantScale、antiquantOffsetOptional非连续Tensor仅支持转置场景并且连续性要求和weight保持一致。
    - `antiquantScale`不同量化模式支持的shape：
      - pertensor[量化模式](../common/量化介绍.md)：(1,)或(1,1)。
      - perchannel[量化模式](../common/量化介绍.md)：输入shape为(1, n)或(n,)。
      - pergroup[量化模式](../common/量化介绍.md)：输入shape为(⌈k/group_size⌉, n)，其中group_size表示k要分组的每组的大小。
      - mx[量化模式](../common/量化介绍.md)：输入shape为(⌈k/group_size⌉, n)，其中group_size表示k要分组的每组的大小，仅支持32。
    ...
    - **约束说明**
      ...
      - 若`weight`数据类型为INT32/FLOAT时，必须配合`aclnnConvertWeightToINT4Pack`接口完成从INT32/FLOAT到紧密排布的INT4/FLOAT4_E2M1的转换，[详情可参考样例](./aclnnConvertWeightToINT4Pack.md)。

  <a id="ascend_950pr_ascend950dt_性能优化建议"></a>

  - **性能优化建议**

    - pertensor[量化模式](../common/量化介绍.md)：推荐使用转置后的`weight`输入。
    - perchannel[量化模式](../common/量化介绍.md)：推荐使用转置后的`weight`输入。
    - pergroup[量化模式](../common/量化介绍.md)和mx[量化模式](../common/量化介绍.md)：推荐使用非转置的`weight`输入。
...
- <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、Atlas 350 加速卡：
