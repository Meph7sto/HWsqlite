...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnInplaceScatterUpdateGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnInplaceScatterUpdate”接口执行计算。
...
- **参数说明**
    ...
    - Atlas 350 加速卡：FLOAT8_E4M3FN、FLOAT8_E5M2、HIFLOAT8等数据类型仅在该型号支持。
