...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用"aclnnQuantGroupedMatmulDequantWeightNZGetWorkspaceSize"接口获取入参并根据流程计算所需workspace大小，再调用"aclnnQuantGroupedMatmulDequantWeightNZ"接口执行计算。
