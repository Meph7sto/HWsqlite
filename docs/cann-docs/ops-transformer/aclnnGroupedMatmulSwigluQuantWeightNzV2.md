...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用"aclnnGroupedMatmulSwigluQuantWeightNzV2GetWorkspaceSize"接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用"aclnnGroupedMatmulSwigluQuantWeightNzV2"接口执行计算。
