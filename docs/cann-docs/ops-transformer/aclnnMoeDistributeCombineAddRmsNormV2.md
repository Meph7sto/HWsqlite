...
- 其他约束：
    ...
    - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、Atlas 350 加速卡：暂不支持动态缩容。

## 调用示例

<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、Atlas 350 加速卡，调用aclnnMoeDistributeDispatchV3和aclnnMoeDistributeCombineAddRmsNormV2接口，具体编译和执行过程请参考[编译与运行样例](../common/编译与运行样例_trans.md)。
