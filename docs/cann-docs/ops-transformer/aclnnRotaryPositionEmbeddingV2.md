...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnRotaryPositionEmbeddingV2GetWorkspaceSize”接口获取入参并根据流程计算所需workspace大小，再调用“aclnnRotaryPositionEmbeddingV2”接口执行计算。
...
- Atlas 350 加速卡：
  ...
  - 输入张量x和输出张量y的shape必须完全相同。
  - 输入张量cos和sin的shape必须完全相同，cos和sin的shape需要与x满[broadcast关系](../common/broadcast关系.md)，且广播的shape必须等于x的shape。
