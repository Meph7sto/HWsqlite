...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用 “aclnnMatmulAlltoAllGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMatmulAlltoAll”接口执行计算。
...
* 空tensor的支持度根据不同设备型号有不同的限制：
  ...
  - Atlas 350 加速卡：仅支持输入x1的第一维度（BS）为0的空tensor，其它空tensor均不支持。
