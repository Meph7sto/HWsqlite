...
- **接口功能**：`aclnnQuantMatmulAllReduceV3`接口是对`aclnnQuantMatmulAllReduceV2`接口的功能扩展, 新增支持低比特通信：MatMul的计算结果依次进行AllToAll通信、ReduceSum计算、AllGather通信、dequant反量化，代替先dequant和pertoken计算、再AllReduce通信的原流程。支持pertensor、perchannel、pertoken[量化方式](../common/量化介绍.md)。
...
- 确定性计算：
  ...
  - Atlas 350 加速卡：`aclnnQuantMatmulAllReduceV3`默认确定性实现。
