...
- 参数key、value使用限制：
  ...
  - Atlas 350 加速卡：数据类型支持FLOAT16、BFLOAT16、INT8、HIFLOAT8、FLOAT8_E4M3FN、INT4(INT32)、FLOAT4_E2M1。
...
- 伪量化参数 antiquantScale和antiquantOffset约束：
  ...
  - Atlas 350 加速卡：数据类型支持FLOAT16、BFLOAT16、FLOAT32。
...
- keyAntiquantScaleOptional：Device侧的aclTensor，数据格式支持ND，kv伪量化参数分离时表示key的反量化因子。如不使用该功能时可传入nullptr。
  ...
  - Atlas 350 加速卡：数据类型支持FLOAT16、BFLOAT16、FLOAT32、FLOAT8_E8M0。支持per-tensor，per-channel，per-token，per-tensor叠加per-head，per-token叠加per-head，per-token叠加使用page attention模式管理scale/offset、per-token叠加per head并使用page attention模式管理scale/offset和per-token-group。
...
- keyAntiquantOffsetOptional：Device侧的aclTensor，数据类型支持FLOAT16、BFLOAT16、FLOAT32。数据格式支持ND，kv伪量化参数分离时表示key的反量化偏移。如果使用该功能其数据类型与shape必须与keyAntiquantScaleOptional保持一致。如不使用该功能时可传入nullptr。
  ...
  - Atlas 350 加速卡：支持per-tensor，per-channel，per-token，per-tensor叠加per-head，per-token叠加per-head，per-token叠加使用page attention模式管理scale/offset、per-token叠加per head并使用page attention模式管理scale/offset。
...
- **当Q_S大于1时**：
  ...
  - page attention场景：
    ...
    - page attention 伪量化场景
      ...
      - Atlas 350 加速卡：支持query dtype为FLOAT16/BFLOAT16，支持key、value dtype为INT8/HIFLOAT8/FLOAT8_E4M3FN/FLOAT4_E2M1/INT4(INT32)。
    - page attention 全量化场景
      - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：不支持query dtype为INT8。
      - Atlas 350 加速卡：支持query和kv cache全部为INT8。
